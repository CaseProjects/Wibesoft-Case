...
namespace Zenject
{
    public class GameObjectContext : RunnableContext
    {
        ...
        [SerializeField] KernelType _kernelType;
        ...
        void InstallBindings(List<MonoBehaviour> injectableMonoBehaviours)
        {
            ...
            if (_kernel == null)
            {
                switch (_kernelType)
                {
                    case KernelType.Tickless:
                        BindMonoKernel<TicklessGameObjectKernel>();
                        break;
                    case KernelType.Default:
                        BindMonoKernel<DefaultGameObjectKernel>();
                        break;
                }
            }
            else
        ...
        private void BindMonoKernel<TMonoKernel>() where TMonoKernel : MonoKernel
        {
            _container.Bind<MonoKernel>().To<TMonoKernel>().FromNewComponentOn(gameObject).AsSingle().NonLazy();
        }
    }
}
